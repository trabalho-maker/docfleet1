import type { Metadata } from "next";
import { notFound } from "next/navigation";
import { getCurrentUser } from "@/features/auth/server/session";
import { EditAssociateSection } from "@/features/associates/components/edit-associate-section";
import { FeedbackAlert } from "@/features/associates/components/feedback-alert";
import {
  canEditAssociate,
  getAssociateAccessMessage,
} from "@/features/associates/lib/associate-authorization";
import {
  AssociateNotFoundError,
  createAssociateService,
} from "@/features/associates/server/associate.service";

export const metadata: Metadata = {
  title: "Editar associado",
  description: "Edição de associados cadastrados no DocFleet.",
};

type EditAssociatePageProps = {
  params: Promise<{
    id: string;
  }>;
};

export default async function EditAssociatePage({
  params,
}: EditAssociatePageProps) {
  const user = await getCurrentUser();
  const canEdit = canEditAssociate(user);
  const accessMessage = getAssociateAccessMessage(user);

  if (!canEdit) {
    return (
      <main className="mx-auto flex w-full max-w-7xl flex-1 px-6 py-10 sm:px-10 lg:px-12 lg:py-16">
        <div className="w-full">
          <FeedbackAlert
            type="error"
            message={accessMessage ?? "Seu perfil não pode editar associados."}
          />
        </div>
      </main>
    );
  }

  const { id } = await params;
  const associateService = createAssociateService();
  let associate;

  try {
    associate = await associateService.getAssociateById(id);
  } catch (error) {
    if (error instanceof AssociateNotFoundError) {
      notFound();
    }

    throw error;
  }

  return (
    <main className="mx-auto flex w-full max-w-7xl flex-1 px-6 py-10 sm:px-10 lg:px-12 lg:py-16">
      <EditAssociateSection
        associateId={associate.id}
        initialValues={{
          name: associate.name,
          cpf: associate.cpf,
          category: associate.category,
          registrationNumber: associate.registrationNumber,
          status: associate.status,
          admissionDate: associate.admissionDate,
          modalidadeAssociado: associate.modalidadeAssociado ?? "",
          cnpjEmpresa: associate.cnpjEmpresa ?? "",
          nomeEmpresa: associate.nomeEmpresa ?? "",
          enderecoCompleto: associate.enderecoCompleto ?? "",
          bairro: associate.bairro ?? "",
          cidade: associate.cidade ?? "",
          estado: associate.estado ?? "",
          cep: associate.cep ?? "",
          profissao: associate.profissao ?? "",
          sexo: associate.sexo ?? "",
          dataNascimento: associate.dataNascimento ?? "",
          nacionalidade: associate.nacionalidade ?? "",
          naturalidade: associate.naturalidade ?? "",
          rg: associate.rg ?? "",
          cnh: associate.cnh ?? "",
          estadoCivil: associate.estadoCivil ?? "",
          nomePai: associate.nomePai ?? "",
          nomeMae: associate.nomeMae ?? "",
          dependentes: associate.dependentes ?? "",
          grauParentesco: associate.grauParentesco ?? "",
          telefone: associate.telefone ?? "",
          celular: associate.celular ?? "",
          email: associate.email ?? "",
          observacoes: associate.observacoes ?? "",
          situacaoFinanceira: associate.situacaoFinanceira ?? "",
          situacaoDocumental: associate.situacaoDocumental ?? "",
          historicoResumo: associate.historicoResumo ?? "",
          fotoUrl: associate.fotoUrl ?? "",
        }}
        userName={user.name}
        userEmail={user.email}
        userRole={user.role}
      />
    </main>
  );
}
